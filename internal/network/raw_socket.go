...
// raw socket으로 주고받을 메시지 구조
type SyncCommand struct {
	Type byte
}


// raw socket으로 메시지를 보내는 구조체
...
func buildIPHeader(payloadLen int, dst [4]byte) []byte {
	...
	ip[2] = byte(totalLen >> 8)     // Total Length (high byte)
	ip[3] = byte(totalLen & 0xff)   // Total Length (low byte)
	ip[4], ip[5] = 0x00, 0x00       // Identification
	ip[6], ip[7] = 0x40, 0x00       // Flags + Fragment Offset
	ip[8] = 64                      // TTL
	ip[9] = syscall.IPPROTO_TCP     // Protocol
	copy(ip[12:16], []byte{127, 0, 0, 1}) // Source IP (loopback)
